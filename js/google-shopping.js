...
console.log("------------Alternative Version (Name is ONLY eBay and has no extended name -------------");
for (let i = 0; i < data.items.length; i++) {
    if (data.items[i].product["author"].name === "eBay" && data.items[i].product["brand"] === "Canon" && data.items[i].product["author"].name.length === 4) {
        console.log(data.items[i]);
    }
}
